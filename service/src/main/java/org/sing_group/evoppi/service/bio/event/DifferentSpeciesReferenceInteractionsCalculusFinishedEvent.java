...
public class DifferentSpeciesReferenceInteractionsCalculusFinishedEvent
extends DifferentSpeciesCalculusEvent
implements Serializable, WorkStepEvent {
  private static final long serialVersionUID = 1L;

  private final Collection<Integer> referenceGeneIds;
  private final Collection<GeneInteraction> referenceInteractions;

  public DifferentSpeciesReferenceInteractionsCalculusFinishedEvent(DifferentSpeciesCalculusEvent event, Set<Integer> referenceGeneIds, Collection<GeneInteraction> referenceInteractions) {
    super(event);
    
    this.referenceGeneIds = new HashSet<>(referenceGeneIds);
  ...
  public IntStream getReferenceGeneIds() {
    return this.referenceGeneIds.stream()
  ...
  public String getDescription() {
    return String.format("%d interactions found with max degree %d for gene %d in interactome %d",
      this.getReferenceInteractions().count(),
      this.getMaxDegree(),
      this.getGeneId(),
