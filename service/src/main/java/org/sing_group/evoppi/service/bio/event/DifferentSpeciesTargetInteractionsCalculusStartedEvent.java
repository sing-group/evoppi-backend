...
public class DifferentSpeciesTargetInteractionsCalculusStartedEvent
extends DifferentSpeciesCalculusEvent
implements Serializable, WorkStepEvent {
  private static final long serialVersionUID = 1L;

  private final Collection<Integer> geneIds;

  public DifferentSpeciesTargetInteractionsCalculusStartedEvent(DifferentSpeciesCalculusEvent event, Collection<Integer> geneIds) {
    super(event);
    
    this.geneIds = new HashSet<>(geneIds);
  }
  ...
  public IntStream getGeneIds() {
    return geneIds.stream().mapToInt(Integer::intValue);
  }

  @Override
  public String getDescription() {
    return String.format("Calculating interactions for %d genes in target interactome %d",
      this.getGeneIds().count(),
