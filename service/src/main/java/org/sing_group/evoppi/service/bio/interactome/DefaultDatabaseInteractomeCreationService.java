...
import org.sing_group.evoppi.service.spi.bio.interactome.DatabaseInteractomeCreationService;
...
public class DefaultDatabaseInteractomeCreationService implements DatabaseInteractomeCreationService {
  @Inject
  private PipelineExecutor executor;

  @Inject
  private DatabaseInteractomeCreationPipeline pipeline;

  @Override
  @Asynchronous
  public void createDatabaseInteractome(
    @Observes(during = TransactionPhase.AFTER_SUCCESS)
    DatabaseInteractomeCreationRequestEvent event
  ) {
    final DatabaseInteractomeCreationConfiguration configuration =
      new DefaultDatabaseInteractomeCreationConfiguration(event.getData(), event.getWorkId());
